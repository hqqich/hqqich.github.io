...
public abstract class LogFactory {
    ...
    static String UNSUPPORTED\_OPERATION\_IN\_JCL\_OVER\_SLF4J = "http://www.slf4j.org/codes.html#unsupported\_operation\_in\_jcl\_over\_slf4j";
...
\[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-0zl7bKRV-1624982501293)(images/搜狗截图20180302221835.png)\]
...
protected WebApplicationContext createRootApplicationContext(
   ...
   if (parent != null) {
      ...
      servletContext.setAttribute(
            WebApplicationContext.ROOT\_WEB\_APPLICATION\_CONTEXT\_ATTRIBUTE, null);
...
\[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-zZm6blLL-1624982501293)(images/搜狗截图20180303145450.png)\]

\[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-ASfPVI3b-1624982501294)(images/搜狗截图20180303145531.png)\]
...
\[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-R5a2j9S7-1624982501294)(images/搜狗截图20180303165113.png)\]
...
538bde63e500        tomcat              "catalina.sh run"        About an hour ago   Exited (143) About an hour ago                       compassionate\_
goldstine
c4f1ac60b3fc        tomcat              "catalina.sh run"        About an hour ago   Exited (143) About an hour ago                       lonely\_fermi
81ec743a5271        tomcat              "catalina.sh run"        About an hour ago   Exited (143) About an hour ago                       sick\_ramanujan


//错误日志
\[root@localhost ~\]# docker logs 42f09819908b
error: database is uninitialized and password option is not specified 
  You need to specify one of MYSQL\_ROOT\_PASSWORD, MYSQL\_ALLOW\_EMPTY\_PASSWORD and MYSQL\_RANDOM\_ROOT\_PASSWORD；这个三个参数必须指定一个

正确的启动

\[root@localhost ~\]# docker run --name mysql01 -e MYSQL\_ROOT\_PASSWORD=123456 -d mysql
...
\[root@localhost ~\]# docker run -p 3306:3306 --name mysql02 -e MYSQL\_ROOT\_PASSWORD=123456 -d mysql
...
docker run --name mysql03 -v /conf/mysql:/etc/mysql/conf.d -e MYSQL\_ROOT\_PASSWORD=my-secret-pw -d mysql:tag
...
\[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-gyO2QcmS-1624982501294)(images/搜狗截图20180305194443.png)\]
...
[mybatis-spring-boot-autoconfigure – Introduction](http://www.mybatis.org/spring-boot-starter/mybatis-spring-boot-autoconfigure/ "mybatis-spring-boot-autoconfigure – Introduction")

### 4、整合SpringData JPA

#### 4.1、SpringData简介

\[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-rJe40FlC-1624982501295)(images/搜狗截图20180306105412.png)\]
...
@Table(name = "tbl\_user") //@Table来指定和哪个数据表对应;如果省略默认表名就是user；
public class User {
...
\[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-MZ9V8diG-1624982501295)(images/搜狗截图20180306145727.png)\]

\[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-ANMb80pK-1624982501296)(images/搜狗截图20180306145855.png)\]
...
[https://github.com/spring-projects/spring-boot/tree/master/spring-boot-samples](https://github.com/spring-projects/spring-boot/tree/master/spring-boot-samples "https://github.com/spring-projects/spring-boot/tree/master/spring-boot-samples")

文章知识点与官方知识档案匹配，可进一步学习相关知识

[云原生入门技能树](https://edu.csdn.net/skill/cloud_native/?utm_source=csdn_ai_skill_tree_blog)[首页](https://edu.csdn.net/skill/cloud_native/?utm_source=csdn_ai_skill_tree_blog)[概览](https://edu.csdn.net/skill/cloud_native/?utm_source=csdn_ai_skill_tree_blog)17234 人正在系统学习中

本文转自 <https://blog.csdn.net/m0_62676056/article/details/120635646>，如有侵权，请联系删除。
